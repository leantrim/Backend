...
router.post('/', Helper_1.upload.single('file'), async (req, res) => {
    ...
    const { name } = path_1.default.parse(file.originalname);
    ...
    const webpBuffer = await (0, sharp_1.default)(file.buffer) // Convert image to webp and compress it
        .webp({ quality: 85 })
        .toBuffer();
    // Upload the file to MinIO
    Helper_1.minioClient.putObject(bucketName, fileName, webpBuffer, (err, etag) => {
        if (err) {
        ...
        Helper_1.minioClient.setObjectTagging(bucketName, fileName, tags, function (err) {
            if (err) {
                return console.log(err);
            }
        });
        Helper_1.minioClient.presignedGetObject(bucketName, fileName, function (err, presignedUrl) {
            if (err) {
